def divide(quotients, divisors):
    ...
    """
    return {x:[y for y in divisors if y%x==0] for x in quotients if any([y%x==0 for y in divisors])}
...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    return sqrt(pow(get_lat(city_a)-get_lat(city_b),2)+pow(get_lon(city_b)-get_lon(city_a),2))
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    if distance(make_city('',lat,lon),city_a)<distance(make_city('',lat,lon),city_b):
